...
import type { DisclosureAlert } from '@/lib/types'
...
function buildAlerts(watchlist: ReturnType<typeof useDemoSession>['watchlist']): DisclosureAlert[] {
  return watchlist.slice(0, 3).map((item, index) => {
    const riskLevel = item.risk_level === 'high' || item.risk_level === 'caution' ? item.risk_level : 'info'
    return {
      id: `${item.corp_code}-${index}`,
      corp_name: item.corp_name,
      risk_level: riskLevel,
      title: riskLevel === 'info' ? '분석 완료' : '공시 기반 위험 신호 확인',
      description:
        riskLevel === 'high'
          ? '작전주 지수 위험 구간입니다. 상세 리포트 확인이 필요합니다.'
          : riskLevel === 'caution'
            ? '주의 항목이 감지되었습니다. 공시 기반 근거를 확인하세요.'
            : '현재 데모 세션 기준 이상 신호가 없습니다.',
      time: index === 0 ? '09:14' : index === 1 ? '08:52' : '08:30',
    }
  })
}

export default function DashboardPage() {
  const { dashboardSummary: summary, watchlist } = useDemoSession()
  const alerts = buildAlerts(watchlist)
  ...
  const averageRisk = summary.count === 0
    ? 0
