...
export default function SearchInput({ onSelect }: Props) {
  const { searchCompanies } = useDemoSession()
  const [query, setQuery] = useState('')
  const results = useMemo(() => searchCompanies(query), [query, searchCompanies])
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <div style={{ position: 'relative' }}>
        ...
        />
        {query && (
          <button onClick={() => setQuery('')} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-tertiary)' }}>
            <IconX size={14} />
          </button>
        )}
      </div>
        ...
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--color-bg-primary)', border: '0.5px solid var(--color-border-secondary)', borderRadius: 'var(--radius-md)', marginTop: 4, zIndex: 50, overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
          ...
          {results.map(c => (
            <button key={c.corp_code} onClick={() => handleSelect(c)} style={{ width: '100%', textAlign: 'left', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', borderBottom: '0.5px solid var(--color-border-tertiary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '-0.03em' }}>{c.corp_name}</p>
                <p className="font-mono" style={{ fontSize: 10, color: 'var(--color-text-tertiary)' }}>{c.stock_code}</p>
              </div>
              <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)', background: 'var(--color-bg-secondary)', padding: '2px 8px', borderRadius: 100 }}>{c.market}</span>
            </button>
          ))}
        </div>
      )}
