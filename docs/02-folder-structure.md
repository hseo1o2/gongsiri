...
## G002 Extension Note
- G002 adds trigger/orchestration code under `agent/src/{cli,scheduler,state,triggers}/`.
- Trigger checkpoint state is local-only and lives in ignored runtime state under `data/agent-runtime/`.
- G002 still does **not** introduce frontend, DB, or notification layout.
